...
pub struct API {
    port: u16,
    database: Database,
    sender: Sender<RoadEvent>,
}

impl API {
    pub async fn new(configuration: &Configuration) -> Result<(Self, Receiver<RoadEvent>)> {
        let (sender, receiver) = channel::<RoadEvent>(10);
        let api = Self {
    ...
    pub async fn run(self) -> Result<()> {
        let app = Router::new()
            ...
            .with_state((Arc::new(RwLock::new(self.database)), self.sender));
